...
TextLayer dateLayer; // The date layer
...
void handle_init(AppContextRef ctx) {
	...
	// 1st line layers
	text_layer_init(&line1.currentLayer, GRect(0, 3, 144, 50));
	text_layer_init(&line1.nextLayer, GRect(144, 3, 144, 50));
	configureBoldLayer(&line1.currentLayer);
	configureBoldLayer(&line1.nextLayer);

	// 2nd layers
	text_layer_init(&line2.currentLayer, GRect(0, 40, 144, 50));
	text_layer_init(&line2.nextLayer, GRect(144, 40, 144, 50));
	configureLightLayer(&line2.currentLayer);
	configureLightLayer(&line2.nextLayer);

	// 3rd layers
	text_layer_init(&line3.currentLayer, GRect(0, 77, 144, 50));
	text_layer_init(&line3.nextLayer, GRect(144, 77, 144, 50));
...
void handle_minute_tick(AppContextRef ctx, PebbleTickEvent *t) {
  ...
  static char dateText[] = "Wednesday 04/23";

  PblTm currentTime;
  get_time(&currentTime);
  string_format_time(dateText, sizeof(dateText), "%A %m/%d", &currentTime);
